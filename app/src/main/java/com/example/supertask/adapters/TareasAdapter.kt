...
class TareasAdapter(
    private val mDataSet: ArrayList<Task>, var click: (task: Task) -> Unit,
    var clickDetalle: (task: Task) -> Unit
) :
    ...
    inner class MainViewHolder(v: View) : RecyclerView.ViewHolder(v) {
        ...
        fun addData(data: Task) {
            ...
            if (data.isPriority) {
                itemView.btPriority.setBackgroundColor(
            ...
            Picasso.get()
                .load(data.image)
                .into(itemView.imgMiTarea)

            itemView.imgMiTarea.setOnClickListener {
